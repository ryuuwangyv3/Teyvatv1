    ...
    <div className="h-full w-full flex flex-col lg:flex-row overflow-hidden bg-[#0b0e14]">
      {lightboxImage && (
        <div className="fixed inset-0 z-[300] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4" onClick={() => setLightboxImage(null)}>
           <img src={lightboxImage} className="max-w-full max-h-[80vh] object-contain rounded-2xl shadow-[0_0_80px_rgba(211,188,142,0.4)] border-2 border-[#d3bc8e]/20" alt="Full view" />
        </div>
      )}

      {/* LEFT PANEL - Sidebar Controls */}
      <aside className="w-full lg:w-[400px] h-1/2 lg:h-full p-6 sm:p-8 bg-[#13182b]/90 border-b lg:border-b-0 lg:border-r border-[#d3bc8e]/10 custom-scrollbar shrink-0 z-20">
        <header className="mb-8">
            <h1 className="text-xl sm:text-2xl font-black genshin-gold mb-1 font-serif tracking-widest flex items-center gap-3">
                <Sparkles className="w-6 h-6 text-amber-500 animate-pulse shadow-[0_0_15px_rgba(245,158,11,0.5)]" /> Vision Alchemy
            </h1>
        ...
        <div className="space-y-6 pb-12">
            <section>
                ...
                <div className="grid grid-cols-2 gap-2">
                    {['Google', 'Pollinations', 'openai', 'OpenRouter'].map(p => (
                        <button key={p} onClick={() => setActiveProvider(p as any)} className={`px-3 py-3 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all ${activeProvider.toLowerCase() === p.toLowerCase() ? 'bg-amber-500/10 border-amber-500 text-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.2)]' : 'bg-black/20 border-white/5 text-gray-600 hover:text-gray-400'}`}>{p}</button>
                    ))}
                </div>
            </section>
            ...
            <section>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                    {filteredModels.map(model => (
                        <button key={model.id} onClick={() => setSelectedModel(model.id)} className={`w-full p-3 rounded-xl border text-left transition-all flex items-center justify-between group ${selectedModel === model.id ? 'bg-amber-500/10 border-amber-500 text-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.2)]' : 'bg-black/20 border-white/5 text-gray-500 hover:border-white/20'}`}>
            ...
            <section>
                ...
                <div className="grid grid-cols-3 gap-2">
                    {ASPECT_RATIOS.map(ratio => (
                        <button key={ratio.id} onClick={() => setSelectedRatio(ratio.id)} className={`p-2.5 rounded-xl border flex flex-col items-center gap-1.5 transition-all ${selectedRatio === ratio.id ? 'bg-amber-500/20 border-amber-500 text-amber-300' : 'bg-black/40 border-white/5 text-gray-500 hover:bg-white/5'}`}>
            ...
            <section className="space-y-4">
                <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full bg-black/60 border border-white/10 rounded-2xl p-4 text-[11px] focus:border-amber-500 outline-none min-h-[100px] resize-none text-white transition-all select-text shadow-inner" placeholder="Describe the vision..." />
            </section>

            <button onClick={() => handleGenerate()} disabled={isGenerating || (!prompt.trim() && mode === 'create')} className="w-full py-5 rounded-2xl flex items-center justify-center gap-3 text-white font-black transition-all shadow-[0_10px_30px_rgba(0,0,0,0.5)] genshin-button disabled:opacity-50 relative group overflow-hidden">
                <div className="absolute inset-0 bg-white/10 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-500 opacity-20"></div>
            ...
            {isGenerating ? (
                <div className="flex flex-col items-center gap-6 animate-in fade-in">
                    <div className="relative w-48 h-48 flex items-center justify-center">
                        <div className="absolute inset-0 rounded-full border-2 border-dashed border-amber-500/20 animate-[spin_10s_linear_infinite]"></div>
                        <div className="absolute inset-4 border border-amber-500/10 rounded-full animate-[spin_5s_linear_infinite_reverse]"></div>
                        <span className="text-4xl font-black genshin-gold font-mono drop-shadow-[0_0_10px_#f59e0b]">{simulatedProgress}%</span>
                    </div>
                    <h2 className="text-xl font-bold genshin-gold uppercase tracking-[0.4em] font-serif text-center drop-shadow-lg">{statusText}</h2>
                </div>
            ) : resultImage ? (
               <div className="w-full animate-in zoom-in-95 duration-700 flex flex-col items-center">
                 <div className="relative rounded-[2.5rem] overflow-hidden border-2 border-amber-500/30 bg-black/60 p-2 cursor-zoom-in group shadow-2xl" onClick={() => setLightboxImage(resultImage)}>
                   <img src={resultImage} className="w-full h-auto max-h-[70vh] object-contain rounded-[2rem] transition-transform duration-700 group-hover:scale-[1.02]" alt="result" />
               ...
               <div className="text-center opacity-30 flex flex-col items-center group">
                  <div className="w-24 h-24 rounded-full border-2 border-dashed border-white/10 flex items-center justify-center mb-6 group-hover:border-amber-500/50 transition-colors">
                      <ImageIcon className="w-10 h-10 text-gray-700 group-hover:text-amber-500 transition-colors" />
                  </div>
